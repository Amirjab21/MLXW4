...
sample_size = 15000 if torch.cuda.is_available() else 3
...
self_attn_layer = MultiHeadAttention(encoder_output_dim=d_model, decoder_dim=d_model, d_model=d_model, num_heads=8, x_attn=False)
cross_attn_layer = MultiHeadAttention(encoder_output_dim=image_encoder_output_dim, decoder_dim=text_dimension_embedding, d_model=d_model, num_heads=8, x_attn=True)
...
batch_size = 80 if torch.cuda.is_available() else 3
